...
func setup(
	...
	# Calculate fragment centroid in texture pixel space
	var fragment_center: Vector2 = calculate_polygon_centroid(cutout_mesh.mask[0])

	# Convert from texture pixel space to world space
	# Get texture dimensions
	var texture := cutout_mesh.texture
	var texture_size := Vector2(texture.get_width(), texture.get_height()) if texture else Vector2(1024, 1024)

	# Normalize to 0-1 range, center around origin, then scale to mesh_size
	var normalized_center := fragment_center / texture_size
	var centered_normalized := normalized_center - Vector2(0.5, 0.5)
	var world_center := centered_normalized * cutout_mesh.mesh_size

	var fragment_offset := Vector3(world_center.x, world_center.y, 0)
	print("fragment_offset (world space): ", fragment_offset)
